...
OAuth via WebView, can be used by both web and mobile/desktop applications[^2].

When using the built-in UIs, the general approach is to send users to
`/_/auth/(login|logout|profile)`. Additionally, for OAuth client-side
applications, like SPAs, mobile or desktop, should use `?pkce_code_challenge=`
(i.e. Proof-Key-for-Code-Exchange) to protect against auth-code interception
attacks.

When building your own UIs, login is handled by:

* for password auth: [`/api/auth/v1/login`](/api/operations/login_handler/),
  which received the credentials directly in return for tokens.
* for OAuth: [`api/auth/v1/oauth/<PROVIDER_NAME>/login`](/api/operations/login_with_external_auth_provider/),
  which constructs a redirect to the external provider including the callback
  address back to your TrailBase instance.
