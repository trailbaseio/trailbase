...
The TypeScript client's `signUp` reflects this: it signs the new user in
whenever no verification is pending.

export const signUpTs = `
// With an email address a confirmation mail is sent and no tokens are issued.
const { verificationRequired } = await client.signUp({
  email: "alice@example.com",
  password: "s3cr3t!",
});

// Username-only accounts have nothing to verify, so we're already signed in.
...
Passing `options: { redirectUri }` controls where the link in the verification
email sends users after confirming. The target needs to be covered by the
