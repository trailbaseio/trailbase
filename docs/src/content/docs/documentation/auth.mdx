...
At an abstract level, whenever a user signs in, either directly into TrailBase
using `email+password` or via an external OAuth2 provider (Google, Microsoft,
...), they receive a set of tokens.
