...
When building your own UIs, [sign-up](#sign-up) is handled by
[`/api/auth/v1/register`](/api/operations/register_user_handler/) and login by:
...
[`/api/auth/v1/register`](/api/operations/register_user_handler/), which takes an
`email` and/or a `username` alongside `password` and `password_repeat`, subject
to the identifier policy above.

Whenever an account carries an email address, TrailBase sends out a verification
email and the account cannot sign in until that address has been confirmed.
Username-only accounts have nothing to verify and can sign in right away.
All clients expose this as a `signUp` method, which registers the account but
never signs it in; follow up with a login where that is applicable.

export const signUpTs = `
// A confirmation mail is sent, the account cannot sign in yet.
await client.signUp({ email: "alice@example.com", password: "s3cr3t!" });

// Username-only accounts have nothing to verify and can sign in right away.
await client.signUp({ username: "alice", password: "s3cr3t!" });
await client.login("alice", "s3cr3t!");
`;

export const signUpDart = `
await client.signUp(email: 'alice@example.com', password: 's3cr3t!');
`;

export const signUpSwift = `
try await client.signUp(password: "s3cr3t!", email: "alice@example.com")
`;

export const signUpKotlin = `
client.signUp(password = "s3cr3t!", email = "alice@example.com")
`;

export const signUpDotnet = `
await client.SignUp(password: "s3cr3t!", email: "alice@example.com");
`;

export const signUpRust = `
client
...
export const signUpGo = `
email := "alice@example.com"
err := client.SignUp(trailbase.SignUpOptions{
	Password: "s3cr3t!",
	Email:    &email,
})
`;

export const signUpPy = `
client.sign_up(password="s3cr3t!", email="alice@example.com")
`;

export const signUpCurl = `
curl
  --header "Content-Type: application/json"
  --request POST
  --data '{ "email": "alice@example.com", "password": "s3cr3t!", "password_repeat": "s3cr3t!" }'
  http://localhost:4000/api/auth/v1/register
`;

<Tabs>
  <TabItem label="JS/TS">
    <Code lang="ts" code={signUpTs} />
  </TabItem>

  <TabItem label="Dart">
    <Code lang="dart" code={signUpDart} />
  </TabItem>

  <TabItem label="Swift">
    <Code lang="swift" code={signUpSwift} />
  </TabItem>

  <TabItem label="Kotlin">
    <Code lang="kotlin" code={signUpKotlin} />
  </TabItem>

  <TabItem label="C#/.NET">
    <Code lang="cs" code={signUpDotnet} />
  </TabItem>

  <TabItem label="Rust">
    <Code lang="rust" code={signUpRust} />
  </TabItem>

  <TabItem label="Go">
    <Code lang="go" code={signUpGo} />
  </TabItem>

  <TabItem label="Python">
    <Code lang="python" code={signUpPy} />
  </TabItem>

  <TabItem label="curl">
    <Code lang="sh" frame="code" code={signUpCurl} />
  </TabItem>
</Tabs>

An optional redirect URI controls where the link in the verification email sends
users after confirming. The target needs to be covered by the
`redirect_uri_allowlist` configuration.

Note that registration reports success even when the account already exists, so
that the endpoint cannot be used to enumerate your users' email addresses.
Password-based sign-up can be disabled altogether under *Settings → Auth*, in
which case registration is rejected with `403`.
