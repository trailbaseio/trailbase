...
`TABLE`s and `VIEW`s through **type-safe** RESTful
...
export const fileUploadInput = githubCodeReference({ path: "crates/schema/src/file.rs", match: "pub struct FileUploadInput"});

Record APIs can also support file uploads and downloads. There's some special
handling in place to only store metadata in the underlying table while the files
themselves are kept in an object store - S3 or simply in your local
file-system.

By adding a `TEXT` column to your table with a `std.FileUpload` or
`std.FileUploads` JSON schema constraint, e.g.
`CHECK(jsonschema('std.FileUpload', <column_name>))`, you instruct TrailBase to
accept files and where to keep the metadata.

Files can then be uploaded via `multipart/form-data` POST requests - recommended
for larger files - or simply by sending the content as <a
href={fileUploadInput}>JSON payload</a>, e.g.: `{<column_name>: {"data": "<base64-contents>"}}`.

Downloading files is different form reading records, since the column only
contain the files' metadata.
Instead use the dedicated GET endpoint for files:
<code>
...
[^2]:
    ...
    `TABLE` the type can be inferred. For everything else you can use
