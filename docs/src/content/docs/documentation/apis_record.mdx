...
By adding a `TEXT` column with a `CHECK(jsonschema('std.FileUpload', <column_name>))`
...
Files can then be uploaded by sending their contents as part of your JSON
requests or `multipart/form-data` POST requests. To upload files with as part of a
JSON request, send a JSON object with the <a href={fileUploadInput}>`FileUploadInput`</a>
layout as value of `<column_name>`. The `data` field should be set to the content of the file
you want to upload, encoded as Base64. For example, a minimal request for uploading a file
looks like `{<column_name>: {"data": "<base64>"}}`.
