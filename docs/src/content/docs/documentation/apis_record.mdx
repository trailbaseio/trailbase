...
- `TABLE`s and `VIEW`s need to be `STRICT`ly[^1] typed to guarantee type-safety
  all the way from your records, via JSON schema, to your client-side language
  bindings [^2].
  `CAST(col AS <TYPE>)` expressions can be used to assert types of computed
  `VIEW` columns.
- `TABLE`s need to have a sequential primary key column to allow for stable
  sorting and thus efficient cursor-based pagination. Either an explicit
  `INTEGER`, UUIDv4 or UUIDv7 `PRIMARY KEY` will do, including `FOREIGN KEY`
  columns.
