...
Schema changes to `TABLE`s or `VIEW`s exposed as record APIs, will also change
the API definition, which requires special attention and may require a well
orchestrated rollout.

For example, adding a new `NOT NULL` column without `DEFAULT`, will require all
clients to start providing a corresponding value during record creation.
...
At the surface, migrations are simply files containing SQL statements in
`trailbase/migrations/` following a naming scheme of
...
Schema changes to `TABLES` or `VIEW`s exposed as APIs will also change the API
definition and thus require special diligence.
...
clients going forward to provide said value, otherwise the API call will fail.
...
In other words, adding and removing `NULL`able columns is a lot simpler and
safer.
...
of the API - potentially using `VIEW`s or `excluded_columns` - move your clients
over, and ensure that no more out-of-date clients exist that we're not ok with
breaking before removing the old API.
