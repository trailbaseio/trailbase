...
TrailBase does **not** provide a built-in revert. Schema migrations are append
only and move strictly forward in time.
They should always be conducted with great care.
If a destructive operation has been rolled out all the way to prod, the data
is gone. You can still undo the schema change by manually applying an inverse
forward migration, i.e. adding a column or table back, but properly undoing the
