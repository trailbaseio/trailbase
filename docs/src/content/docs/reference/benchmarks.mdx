...
<Aside type="note" title="Update: TrailBase v0.3+">
...
more than 20 times faster than SupaBase[^5], and comfortably 7 times faster
than PocketBase [^1].
...
between 120MB and 150MB  making them suitable for a tiny VPS or a toaster.
...
For TrailBase, median latencies for reads were 17x (C#) and insertions almost 7x faster.
The latter is well in line with the throughput results we've seen above.
...
This likely means that the bottleneck for PocketBase is on the server-side.
