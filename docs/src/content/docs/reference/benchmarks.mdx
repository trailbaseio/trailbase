...
[TrailBase went through a transition from a V8-based runtime to a WASM one, which can support many guest languges such as JS, TS and Rust](/blog/switching_to_a_wasm_runtime).

V8's execution with just-in-time (JIT) compilation is quite speedy and was about
