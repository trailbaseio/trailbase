...
If your web app uses client-side routing (e.g., React Router, Vue Router, TanStack Router),
enable SPA fallback mode to avoid 404 errors when users refresh the page on routes like `/user/profile`:

```bash
$ trail run --public-dir dist --public-dir-spa
```

With `--public-dir-spa` enabled:
- **Route requests** (paths without file extensions like `/about`, `/user/profile`) → serve `index.html`
- **File requests** (paths with extensions like `/favicon.ico`, `/assets/style.css`) → serve the file or return 404
