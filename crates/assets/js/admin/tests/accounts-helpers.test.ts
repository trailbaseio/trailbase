...
describe("formatAccountTime", () => {
  ...
  test("uses a stable label for sub-minute values", () => {
    expect(formatAccountTime(1000n, now - 59_600)).toBe("just now");
    expect(formatAccountTime(1000n, now + 59_600)).toBe("just now");
    expect(formatAccountTime(1000n, now - 400)).toBe("just now");
  });
});
