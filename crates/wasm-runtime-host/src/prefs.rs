...
pub(crate) async fn handle_prefs_request2(
  ...
  let prefs_request = match to_request(request).await {
    Ok(request) => request,
    Err(err) => {
      return to_response2(PrefsResponse::Error(err));
    }
  };
  ...
  return match prefs_request {
    PrefsRequest::Get { key } => {
      ...
      match get_pref(conn, component_name, key).await {
        Ok(value) => to_response2(PrefsResponse::Value(value)),
        Err(err) => to_response2(PrefsResponse::Error(err.to_string())),
      }
    }
    PrefsRequest::Set { key, value } => {
      ...
      match set_pref(conn, component_name, key, value).await {
        Ok(_) => to_response2(PrefsResponse::Ok),
        Err(err) => to_response2(PrefsResponse::Error(err.to_string())),
      }
    }
  };
