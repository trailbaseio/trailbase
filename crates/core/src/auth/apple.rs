...
where
  ...
  Fut: Future<Output = Result<JwkSet, String>> + Send,
{
  type Cached = Option<(
    SystemTime,
    Shared<BoxFuture<'static, Result<JwkSet, String>>>,
  )>;
...
mod tests {
  ...
  async fn test_cache() {
    ...
    type ResultType = Result<JwkSet, String>;
    ...
    *RESULT.lock() = Ok(JwkSet { keys: vec![] });
