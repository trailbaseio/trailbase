-- Stop using `email` column as scratch space for yet unverified emails. This
-- way we guarantee that `email` may only contain validated entries. Otherwise,
-- logged in users may appear with unverified emails until their auth token
-- expires.

CREATE TABLE IF NOT EXISTS _new_with_unverified_email (
  -- We only check `is_uuid` rather than `is_uuid_v4` to preserve user
  -- previously created as uuiv7.
  id                               BLOB PRIMARY KEY NOT NULL CHECK(is_uuid(id)) DEFAULT (uuid_v4()),

  -- The email column may only ever contain validated email addresses.
  email                            TEXT COLLATE NOCASE CHECK(is_email(email)),
  unverified_email                 TEXT COLLATE NOCASE CHECK(is_email(email)),

  username                         TEXT COLLATE NOCASE,
  password_hash                    TEXT,
  admin                            INTEGER DEFAULT FALSE NOT NULL,

  -- TOTP secret for authenticator.
  totp_secret                      TEXT,

  created                          INTEGER DEFAULT (UNIXEPOCH()) NOT NULL,
  updated                          INTEGER DEFAULT (UNIXEPOCH()) NOT NULL,

  -- OAuth metadata
  --
  -- provider_id maps to `proto.config.OAuthProviderId`, where `0 == undefined`.
  provider_id                      INTEGER DEFAULT 0 NOT NULL,
  -- The external provider's id for the user.
  provider_user_id                 TEXT,
  -- Link to an external avatar image for oauth providers only.
  provider_avatar_url              TEXT

  -- QUESTION: Should we add a TABLE CHECK to make sure entry is either local,
  -- anonymous or OTP.
) STRICT;

INSERT INTO _new_with_unverified_email (
    id,
    email,
    unverified_email,
    username,
    password_hash,
    admin,
    totp_secret,
    created,
    updated,
    provider_id,
    provider_user_id,
    provider_avatar_url
  )
  SELECT
    id,
    CASE
      WHEN verified = TRUE THEN email
      ELSE NULL
    END AS email,
    CASE
      WHEN verified = FALSE THEN email
      ELSE NULL
    END AS unverified_email,
    username,
    password_hash,
    admin,
    totp_secret,
    created,
    updated,
    provider_id,
    provider_user_id,
    provider_avatar_url
  FROM _user;

-- Turn ON legacy behavior to avoid issues with VIEWs referencing the _user
-- table. FOREIGN_KEY constraints are handled by refinery.
PRAGMA legacy_alter_table=ON;

DROP TABLE _user;
ALTER TABLE _new_with_unverified_email RENAME TO _user;

-- Turn OFF legacy behavior.
PRAGMA legacy_alter_table=OFF;

-- Re-create INDEXes and TRIGGERs removed by `DROP TABLE`.
CREATE UNIQUE INDEX __user__email_index ON _user (email);
CREATE UNIQUE INDEX __user__username_index ON _user (username);
CREATE UNIQUE INDEX __user__provider_ids_index ON _user (provider_id, provider_user_id);

CREATE TRIGGER __user__updated_trigger AFTER UPDATE ON _user FOR EACH ROW
  BEGIN
    UPDATE _user SET updated = UNIXEPOCH() WHERE id = OLD.id;
  END;
