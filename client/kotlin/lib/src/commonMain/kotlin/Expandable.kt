...
data class Expandable<T> private constructor(
    val id: RecordId,
    val data: T? = null,
) {
    companion object {
    ...
    override fun deserialize(decoder: Decoder): Expandable<T> =
        try {
            ...
            throw SerializationException(
                """Failed to parse Expandable. Have you set `expand=listOf("<column_name>")` in the `read` or `list` call to expand the column?""",
                e
            )
        }
}
